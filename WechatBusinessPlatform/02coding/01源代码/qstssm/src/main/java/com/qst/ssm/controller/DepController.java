...
public class DepController {
    ...
    /**
     * 加载部门数据,向客户端输出JSON格式数据
     * @return
     */
    @RequestMapping(value = "load_json_data",produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    public String loadDepJSONData() {
        List<Dep> depList = depService.queryDep();
        //返回部门数据
        return new JSONData(depList).toJSONString();
    }
    /**
     * 加载部门数据,向客户端输出XML格式数据
     * @return
     */
    @RequestMapping(value = "load_xml_data",produces = MediaType.TEXT_XML_VALUE)
    @ResponseBody
    public List<Dep> loadDepXMLData() {
        List<Dep> depList = depService.queryDep();
        //返回部门数据
        return depList;
    }
